...
class FoodGeneralCrawler:
    ...
    def foodInformationGetter(self, input_data, offset):
        ...
        conn.request(
            "GET",
            "/restaurants/list?restaurant_tagcategory_standalone=10591&lunit=km&restaurant_tagcategory=10591&limit=30&prices_restaurants=10953%252C10955&restaurant_mealtype=10598%252C10599&currency=USD&lang=en_US&offset="
            + str(offset)
            + "&location_id="
            + str(location_id),
            headers=headers,
        ...
        for one_data in dict_data_data:
            try:
                ...
                if -1 == one_data.get("price", -1):  # cost가 없으면 그 데이터는 넣지 않는다.
                    ...
                    self.moneyGeneralTrimmer(one_data.get("price"))
                ):  # cost가 1이면 안넣기
                    continue
                else:
                    food_data.append(
                        int(self.moneyGeneralTrimmer(one_data.get("price")))
                    )  # 넣기
